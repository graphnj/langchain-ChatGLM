...
from textsplitter.file_converter import (
    BaseConverter,
    DocxToTextConverter,
...
def convert_files_to_dicts(
    ...
    for suffix, paths in suffix2paths.items():
        for path in paths:
            ...
            list_documents = suffix2converter[suffix].run(
                file_paths=path,
                remove_numeric_tables= None,  # type: ignore
                ...
                if split_paragraphs:
                    for para in text.split("\n"):
                        ...
                        if split_answers:
                            ...
                            meta_data = {"filename": path.name, "answer": answer, "file_directory": Path(path).parent, "source": path}
                            # Add image list parsed from docx into meta
                            if document["meta"] is not None and "images" in document["meta"]:
                                meta_data["images"] = document["images"]

                            #documents.append({"page_content": query, "meta": meta_data})
                            documents.append(Document(page_content=query, metadata=document["meta_data"]))
                        else:
                            meta_data = {
                                "filename": path.name,  "file_directory": Path(path).parent, "source": path
                            }
                            # Add image list parsed from docx into meta
                            if document["meta"] is not None and "images" in document["meta"]:
                                meta_data["images"] = document["documents"][0]["meta"]["images"]
                            #documents.append({"page_content": para, "meta": meta_data})
                            documents.append(Document(page_content=para, metadata=meta_data))
                else:
                    documents.append(
                        Document(page_content=text, metadata=document["meta"])
                    )
    return documents
...
def tika_convert_files_to_dicts(
    ...
    for path in file_paths:
        ...
        if split_paragraphs:
                ...
                if paras:
                    ...
                    for para in paras:
                        ...
                        if (merge_short and para_is_short) or (merge_lowercase and para_is_lowercase):
                            ...
                            if last_para:
                                documents.append({"page_content": last_para, "meta": meta})
                            last_para = para
            ...
            if clean_func:
                text = clean_func(text)
            documents.append({"page_content": text, "meta": meta})
