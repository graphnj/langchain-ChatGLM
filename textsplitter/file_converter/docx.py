        ...
        for i in range(len(file.paragraphs)):
            ...
            if paragraph.text != "":
                ...
                if image_list is None and bool(text_dict):
                    ...
                    continue
                # This part will parse the docs files with tables
        for i in range(len(file.tables)):
            table1 = file.tables[i]
            mat = []  # 存储行
            mat_rows = []  # 临时存储行
            head = []  # 存储表头
            columns = table1._column_count  # 列的长度
            rows = len(table1.rows)  # 行的长度

            for c in range(0, columns):
                cell = table1.cell(0, c)
            ...
            for r in range(1, rows):
                # print(f"方法2处理第{r}行数据")
                mat_rows.append(table1.row_cells(r))
